<template>
    ...
    <div id="main">
        ...
        <section id="one">
            <div class="inner">
                <header class="major">
                    <h2>Database of Ambulance and Hospitals</h2>
                </header>
                <p></p>
                <select name="demo-category" id="demo-category" v-model="district" @change="onChange($event)">
                    ...
                    <table>
                        ...
                        <tbody v-if="err === false">
                            <tr v-for="ambHos in ambulanceHospitals" v-bind:key="ambHos.id">
                                ...
                                <td>{{ambHos.name}} <span v-if="ambHos.verified"><img src="images/verified.png" alt="" srcset="" class="verified"></span></td>
                                <td>{{ambHos.address}}</td>
                                <td><a :href="`tel:${ambHos.phone}`">{{ambHos.phone}}</a></td>
                            </tr>
    ...
    export default {
        ...
        methods: {
            async onChange(e) {
                ...
                try {
                    const res = await axios.get(`http://localhost:5000/api/v1/ambulance-hospitals/${e.target.value}`)
