                    ...
                    <table>
                        ...
                        <tbody v-if="err === false">
                            <tr v-for="doctor in docData" v-bind:key="doctor.id">
                                <td>{{doctor.name}} <span v-if="doctor.doctorVerified"><img src="images/verified.png" alt="" srcset="" class="verified"></span></td>
    ...
    export default {
        ...
        methods: {
            async onChange(e) {
                ...
                try {
                    const res = await axios.get(`http://localhost:5000/api/v1/doctor/${e.target.value}`)
