                    ...
                    <table>
                        ...
                        <tbody v-if="err === false">
                            <tr v-for="ngo in ngos" v-bind:key="ngo.id">
                                <td>{{ngo.name}} <span v-if="ngo.verified"><img src="images/verified.png" alt="" srcset="" class="verified"></span></td>
                                <td>{{ngo.ngovolunteer}}</td>
                                <td>{{ngo.organization}}</td>
                                <td>{{ngo.address}}</td>
                                <td><a :href="`tel:${ngo.phone}`">{{ngo.phone}}</a></td>
                            </tr>
    ...
    export default {
        ...
        methods: {
            async onChange(e) {
                ...
                try {
                    const res = await axios.get(`http://localhost:5000/api/v1/ngos/${e.target.value}`)
                    this.ngos = res.data
