                    ...
                    <table>
                        ...
                        <tbody v-if="err === false">
                            <tr v-for="o in other" v-bind:key="o.id">
                                <td>{{o.details}}</td>
                                <td v-if="o.file !== undefined"><a :href="`${getUrlfromId(o.file.asset._ref + '')}`">Download</a></td>
                                <td v-else>No File Available</td>
                                <td v-if="o.image !== undefined"><a :href="`${imageUrlFor(o.image.asset._ref + '')}`">Download</a></td>
                                <td v-else>No File Available</td>
                            </tr>
    ...
    export default {
        ...
        methods: {
            async loadData() {
                this.isLoading = true
                try {
                    const res = await axios.get(`http://localhost:5000/api/v1/other/`)
                    this.other = res.data
