                    ...
                    <table>
                        ...
                        <tbody v-if="err === false">
                            <tr v-for="blood in bloods" v-bind:key="blood.id">
                                <td>{{blood.name}} <span v-if="blood.verified"><img src="images/verified.png" alt="" srcset="" class="verified"></span></td>
                                <td>{{blood.organization}}</td>
                                <td>{{blood.address}}</td>
                                <td><a :href="`tel:${blood.phone}`">{{blood.phone}}</a></td>
                            </tr>
    ...
    export default {
        ...
        methods: {
            async onChange(e) {
                ...
                try {
                    const res = await axios.get(`http://localhost:5000/api/v1/blood/${e.target.value}`)
